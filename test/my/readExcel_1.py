...
for i in range(1,nrows):
    ...
    if('INPATIENT_MEDICAL_RECORD' in cell_value):                
        sql+=cell_value1+','
        sql5+=cell_value2+' '+cell_value1+','+'\n'
        count+=1
    if('GENERAL_OPERATION_RECORDS' in cell_value):
        sql1+=cell_value1+','
        sql6+=cell_value2+' '+cell_value1+','+'\n'
        count1+=1
    if('DIAGNOSIS' in cell_value):
        ...
        sql2+=cell_value1+','
        sql7+=cell_value2+' '+cell_value1+','+'\n'
        count2+=1
    if(operator.eq('PATIENT',cell_value)):
        sql3+=cell_value1+','
        sql8+=cell_value2+' '+cell_value1+','+'\n'
        count3+=1
print('insert into HQMS' + sql+')'+'\n'+'values'+sql5+' from INPATIENT_MEDICAL_RECORD'+'\n')
print('insert into HQMS' + sql1+')'+'\n'+'values'+sql6+' from GENERAL_OPERATION_RECORDS'+'\n')
print('insert into HQMS' + sql2+')'+'\n'+'values'+sql7+' from DIAGNOSIS'+'\n')
print('insert into HQMS' + sql3+')'+'\n'+'values'+sql8+' from PATIENT'+'\n')
